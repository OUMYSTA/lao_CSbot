<template>
  <div id="chat" class="flex flex-col justify-end bg-gray-50 h-96 mx-4 my-4 rounded-lg shadow-lg">
    <div ref="chatContainer" class="carousel carousel-vertical overflow-y-auto">
      <div class="px-8" v-for="(msg, index) in messages" :key="index">
        <div :class="msg.isUser ? 'chat chat-end' : 'chat chat-start'" class="carousel-item">
          ...
          <div
            class="chat-bubble text-gray-900 mb-4 text-xl p-4 drop-shadow shadow-md"
            :class="msg.isUser ? 'bg-[#FFC107] text-white' : 'bg-gray-200'"
          >
            {{ msg.text }}
          </div>
        </div>
    ...
    <div class="flex items-center justify-center w-full px-4 py-2 rounded-b-lg bg-gray-600">
      ...
      <button @click="sendMessage" class="ml-2 btn bg-[#FFC107] btn-circle border-0">
        <img
          src="https://cdn-user-icons.flaticon.com/70116/70116157/1720718118692.svg?token=exp=1720719043~hmac=acac7f232d2cd863015bfa0be77e80be"
          alt="share"
          width="22px"
          height="22px"
        />
